...
import com.yonyou.ucf.mdd.ext.meta.model.MetaColumnModel;
import com.yonyou.ucf.mdd.ext.meta.model.MetaModel;
...
public class MetaModelService {
	...
	public List<MetaModel> getModels(List<String> fullnames,Map<String,List<String>> fields,boolean onlyOneLevel) throws Exception {
		try{
			...
			if(null==fullnames || fullnames.size()==0) fullnames=MddBaseContext.uimetaDao().selectList("com.yonyoucloud.uretail.bill.meta.dao.design.getMetaModels",params);//TODO sql没粘过来
	...
	private MetaModel createMetaModelByName(String entityName, boolean isMain, boolean isRelation) throws Exception {
		...
		if(entity!=null){
			...
			if(!CollectionUtils.isEmpty(entity.attributes())){
				List<MetaColumnModel> columns=new ArrayList<MetaColumnModel>();
